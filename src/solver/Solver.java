    ...
    public HashMap<Edge, Double> calculateGraphEdgeLengths() {
        ...
        for (Edge edge : graphEdgeRoutes.keySet()) {
            ...
            for (int i = 1; i < route.length; i++) {
                ...
                double[] XY1 = data.cellToLocation(cell1);
                double[] XY2 = data.cellToLocation(cell2);
                if (data.getProjectionVersion() == 1) {
                    // Calculate distance
                    double p = 0.017453292519943295;
                    double a = 0.5 - Math.cos((XY2[0] - XY1[0]) * p) / 2 + Math.cos(XY1[0] * p) * Math.cos(XY2[0] * p) * (1 - Math.cos((XY2[1] - XY1[1]) * p)) / 2;
                    distance += 12742 * Math.asin(Math.sqrt(a));
                } else if (data.getProjectionVersion() == 2) {
                    double deltaX = Math.abs(XY1[0] - XY2[0]);
                    double deltaY = Math.abs(XY1[1] - XY2[1]);
                    distance += Math.sqrt(deltaX * deltaX + deltaY * deltaY) / 1000.0;
                }
