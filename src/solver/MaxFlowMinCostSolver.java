 ...
 * @author danie
 */
public class MaxFlowMinCostSolver {
    // Instance fields
    ...
    private double inputTotalStored;
    ...
    // Constructor
    public MaxFlowMinCostSolver(HashMap<Edge, Integer> hmEdges, HashMap<Source, Integer> hmSources, HashMap<Sink, Integer> hmSinks,
    ...
    double inputTotalStored, String basePath, String dataset, String scenario, double crf, HashMap<Edge, Double> edgeConstructionCosts, 
    LinearComponent[] linComp) {
        ...
        this.gui = gui;
        this.inputTotalStored = inputTotalStored;
        ...
        hmEdgesFinalTrends = new HashMap<>();
    }
        ...
        try {
            ...
            for(int e = 0; e < hmEdgesAveVol.keySet().size(); e++) {
                for(int t = 0; t < linComp.length; t++) {
                    yet[e][t] = cplex.intVar(0, 1, "edgYN [" + e + "]");
                }
            }
...
//                cplex.addLe(expr, 1);
            ...
            /* The amount captured and stored must equal the amount stored in the MF formulation */
            // create an IloLinearNumExpr
            IloLinearNumExpr constraintEExpr = cplex.linearNumExpr();
            for(int cell : hmCells) {
                // add the sink terms (has to be done no matter what)
                addSinkTerms(constraintEExpr, cell);
            }
            // add the negative Max flow sum amount
            IloNumVar inputMaxFlow = cplex.numVar(inputTotalStored, inputTotalStored);
            constraintEExpr.addTerm(1, inputMaxFlow);
            cplex.addEq(constraintEExpr, 0);
           // Integer Linear Program documentation lookup
...
//                System.out.println(cplex.getStatus());  // useful debugging line
                ...
                for(Edge edge : hmEdgesAveVol.keySet()) {
                    if(((double)hmEdges.get(edge) / numbOfRealizations) >= gui.getHmPercentToDisp()) {
                        ...
                        for(int t = 0; t < linComp.length; t++) {
                            ...
                            if(cplex.getValue(pet[count][t]) > 0) {
                                Integer[] trendsAndVerts = {t, edge.v1, edge.v2};
                                hmEdgesFinalTrends.put(edge, trendsAndVerts);
                            }
                        }
                        if(hmEdgesFinalTrends.get(edge) == null) {
                            Integer[] trendsAndVerts = {0, edge.v1, edge.v2};
                            hmEdgesFinalTrends.put(edge, trendsAndVerts); // debug code
                        }
                ...
                System.out.println("Not feasible - Max Flow Min Cost Solver");
            }      
    ...
    public HashMap<Edge, Integer[]> getHmEdgesFinalTrends() {
        return hmEdgesFinalTrends;
    }
